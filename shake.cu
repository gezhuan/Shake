...
__global__ void MoveBotPar(size_t * BotPar,  DEM::DynParticleCU * DPar, UdCu * UC, DEM::dem_aux const * demaux)
{
...
void Setup (DEM::Domain & d2, void * UD) //function that is going to be called every time step
{
...
int main(int argc, char **argv) try
{
    ...
    for (size_t np=0;np<d2.Particles.Size();np++)
    {
        ...
        {
            d2.Particles[np]->FixFree = true;
            d2.Particles[np]->FixVeloc();
            d2.Particles[np]->InitializeVelocity(dt);
            hBotPar.push_back(np);
            dat.BPar.Push(np);
